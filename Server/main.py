...
async def registeration(request:Request,db:Session=Depends(get_db)):
    try:
        ...
        upload_aadhar_folder = os.path.join(upload_folder, "aadhar_Card")
        upload_pan_folder = os.path.join(upload_folder, "pan_Card")
        upload_photo_folder = os.path.join(upload_folder, "photo")
        upload_certificates_folder = os.path.join(upload_folder, "certificates")
        upload_resume_folder = os.path.join(upload_folder, "resume")
        upload_passport_folder = os.path.join(upload_folder, "passport")

            # Create upload folders if they don't exist
        os.makedirs(upload_folder, exist_ok=True)
        os.makedirs(upload_aadhar_folder, exist_ok=True)
        os.makedirs(upload_pan_folder, exist_ok=True)
        os.makedirs(upload_photo_folder, exist_ok=True)
        os.makedirs(upload_certificates_folder, exist_ok=True)
        os.makedirs(upload_resume_folder, exist_ok=True)
        os.makedirs(upload_passport_folder, exist_ok=True)
...
async def get_file(file_type: str, file_name: str):
    try:
        if file_type == "declaration":
            file_path = f"uploads/declaration/{file_name}"
        elif file_type == "offerletter":
            file_path = f"uploads/offerletter/{file_name}"
        elif file_type == "internletter":
            file_path = f"uploads/internletter/{file_name}"
        elif file_type == "feedback":
            file_path = f"uploads/feedback/{file_name}"
        else:
...
async def placeddata(request: Request, db: Session = Depends(get_db), current_user: str = Depends(get_current_user)):
    try:
        ...
        if all(file is not None for file in [declaration_file, feedback_file, internletter_file, offerletter_file]):
            ...
            token_declaration = str(uuid.uuid4()) + '.pdf'
            token_feedback = str(uuid.uuid4()) + '.pdf'
            token_internletter = str(uuid.uuid4()) + '.pdf'
            token_offerletter = str(uuid.uuid4()) + '.pdf'
